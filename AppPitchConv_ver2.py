...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.clsApp05_RunConv_SIDE_SPACE_for_Run = 200    # << 変換実行ボタンの、説明の横位置からの左詰めスペース
        ...
        self.clsApp05_RunConv_spinbox01 = tk.Spinbox(self,
        ...
        #self.clsApp05_RunConv_spinbox01.bind("<Return>", self.spinbox_bind_enter)
        #self.clsApp05_RunConv_spinbox01.bind("<KeyRelease>", self.spinbox_bind_input_num)
        self.clsApp05_RunConv_spinbox01.place(x = self.clsApp05_RunConv_POS_X + self.clsApp05_RunConv_SIDE_SPACE,
        ...
        self.clsApp05_RunConv_button.place(x = self.clsApp05_RunConv_POS_X + self.clsApp05_RunConv_SIDE_SPACE + self.clsApp05_RunConv_SIDE_SPACE_for_Run,
                                           y = self.clsApp05_RunConv_POS_Y + self.clsApp05_RunConv_LINE_SPACE,
                                           anchor=tk.W)
    ...
    def button_click_RunConv(self):
        ...
        for each_file in self.file_list:
            ...
            pitch_data = 0.1
            span_data = int(1/pitch_data)

            data_length = int(re.findall(r'\d+', read_file[174])[0])
            #data_length = int(read_file[174])

            new_data_length = int(data_length/span_data)

            total_data_length = 175 + data_length

            read_file[60] = f"{pitch_data}\n"
            read_file[174] = f"{new_data_length}\n"

            new_pitch_data_removed_lines = read_file[175:total_data_length+1:span_data]
            new_pitch_data_filled_lines = []
            for item in new_pitch_data_removed_lines:
                left, right = item.strip().split(',')
                new_right = int(int(right)/ 10)
                new_pitch_data_filled_lines.append(f'{left},{new_right}\n')

            new_all_data = read_file[:175] + new_pitch_data_filled_lines

            with open(self.save_file, mode="w", encoding="SHIFT-JIS") as new_file:
                new_file.writelines(new_all_data)
